...
const (
	...
	// Rate threshold used to determine if a cluster is balanced
	MAX_UNBALANCE_RATE float64 = 2.0
)
...
func (r *RedisClusterReconciler) ReconcileClusterObject(ctx context.Context, req ctrl.Request, redisCluster *redisv1.RedisCluster) (ctrl.Result, error) {
	...
	r.checkStorageConfigConsistency(ctx, redisCluster, true)

	if redisCluster.Spec.Replicas == 1 {
...
func (r *RedisClusterReconciler) checkStorageConfigConsistency(ctx context.Context, redisCluster *redisv1.RedisCluster, updateRDCL bool) bool {
	...
	updateToError := false
	if !redisCluster.Spec.Ephemeral && (redisCluster.Spec.Storage != "" || redisCluster.Spec.StorageClassName != "") {
		...
		if sset != nil && sset.Spec.VolumeClaimTemplates != nil && len(sset.Spec.VolumeClaimTemplates) > 0 {
			// Get the current value of storage configured in the statefulset object
			...
			// Validate if storage capacity has changed
			if redisCluster.Spec.Storage != stsStorage {
				updateToError = true
				if updateRDCL {
					err = errors.NewBadRequest("spec: Forbidden: updates to redisCluster.Spec.Storage field are forbidden")
					r.LogError(redisCluster.NamespacedName(), err, "spec: Forbidden: updates to redisCluster.Spec.Storage field are forbidden")
			...
			if sset.Spec.VolumeClaimTemplates[0].Spec.StorageClassName != nil {
				...
				r.LogInfo(redisCluster.NamespacedName(), "Current statefulset Storage configuration", "Spec.VolumeClaimTemplates[0].Spec.StorageClassName", stsStorageClassName)

				// Validate if StorageClassName value has changed
				if redisCluster.Spec.StorageClassName != stsStorageClassName {
					updateToError = true
					if updateRDCL {
						err = errors.NewBadRequest("spec: Forbidden: updates to redisCluster.Spec.StorageClassName field are forbidden")
						r.LogError(redisCluster.NamespacedName(), err, "spec: Forbidden: updates to redisCluster.Spec.StorageClassName field are forbidden")
						r.Recorder.Event(redisCluster, "Warning", "ClusterError", err.Error())
						redisCluster.Status.Status = redisv1.StatusError
					}
				}
			...
			updateToError = true
			if updateRDCL {
				err = errors.NewBadRequest("spec: Error: non ephemeral cluster without VolumeClaimTemplates configured")
				r.LogError(redisCluster.NamespacedName(), err, "spec: Error: non ephemeral cluster without VolumeClaimTemplates configured")
				r.Recorder.Event(redisCluster, "Warning", "ClusterError", err.Error())
				redisCluster.Status.Status = redisv1.StatusError
			}
		}
	}
	return updateToError
}
...
func (r *RedisClusterReconciler) reconcileStatusReady(ctx context.Context, redisCluster *redisv1.RedisCluster) (bool, time.Duration) {
	...
	if podsReady {
		...
		for _, node := range clusterNodes.Nodes {
			for _, friend := range node.Friends() {
				if friend.HasFlag("fail") || friend.HasFlag("noaddr") {
					cleanupNeeded = true
					r.LogInfo(redisCluster.NamespacedName(), "fail|noaddr flag found. Cleanup needed")
					break
				}
			}
			if cleanupNeeded {
		...
		if cleanupNeeded {
			...
			r.LogInfo(redisCluster.NamespacedName(), "Complete cleaning of missing nodes")
...
func (r *RedisClusterReconciler) AssignMissingSlots(ctx context.Context, redisCluster *redisv1.RedisCluster) error {
	...
	slotsPerNode := int(math.Ceil(float64(16384 / masterLen)))
...
func (r *RedisClusterReconciler) isClusterBalanced(ctx context.Context, redisCluster *redisv1.RedisCluster) (bool, error) {
	...
	// Check if cluster unbalance rate passes the threshold
	rate, err := clusterNodes.GetUnbalanceRate(ctx)
	if err != nil {
		return false, err
	}
	r.LogInfo(redisCluster.NamespacedName(), "Check cluster balance", "unbalance rate", rate)

	return rate < MAX_UNBALANCE_RATE, nil
}
...
func (r *RedisClusterReconciler) CreateStatefulSet(ctx context.Context, req ctrl.Request, spec redisv1.RedisClusterSpec, labels map[string]string, annotations map[string]string, configmap *corev1.ConfigMap) (*v1.StatefulSet, error) {
	...
	if inferResources {
		statefulSetWithInferredResources, err := r.inferResources(ctx, req, spec, configmap, statefulSet)
		if err != nil {
			return nil, err
		}
...
func (r *RedisClusterReconciler) inferResources(ctx context.Context, req ctrl.Request, spec redisv1.RedisClusterSpec, configmap *corev1.ConfigMap, statefulSet *v1.StatefulSet) (*v1.StatefulSet, error) {
	config := spec.Config
	...
	if err != nil {
		if errors.IsNotFound(err) {
			...
			if createSsetError != nil {
				r.LogError(redisCluster.NamespacedName(), err, "Error when creating StatefulSet")
				return immediateRequeue, err
			}
