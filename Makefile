...
help: ## Display this help.
	@awk 'BEGIN { FS = ":.*##"; category = "General"; order_count = 1; order[1] = category; seen[category] = 1 } \
		/^##@/ { \
			category = substr($$0, 5); \
			if (!(category in seen)) { \
				seen[category] = 1; \
				order[++order_count] = category; \
			} \
			next; \
		} \
		/^[a-zA-Z_0-9-]+:.*?##/ { \
			target = $$1; \
			description = $$2; \
			entries[category, ++entry_count[category]] = target SUBSEP description; \
			if (length(target) > max_width[category]) { \
				max_width[category] = length(target); \
			} \
		} \
		END { \
			printf "\nUsage:\n  make \033[36m<target>\033[0m\n"; \
			for (category_index = 1; category_index <= order_count; category_index++) { \
				current = order[category_index]; \
				if (entry_count[current] == 0) { \
					continue; \
				} \
				printf "\n\033[1m%s\033[0m\n", current; \
				for (entry = 1; entry <= entry_count[current]; entry++) { \
					split(entries[current, entry], parts, SUBSEP); \
					printf "  \033[36m%-*s\033[0m %s\n", max_width[current], parts[1], parts[2]; \
				} \
			} \
		}' $(MAKEFILE_LIST)
...
CI_COVER_PROFILE ?= $(CI_ARTIFACTS_DIR)/cover.out
STATIC_ANALYSIS_STATUS_FILE ?= $(CI_ARTIFACTS_DIR)/ci-static-analysis.status
UNIT_TEST_STATUS_FILE ?= $(CI_ARTIFACTS_DIR)/ci-unit-tests.status
INTEGRATION_TEST_STATUS_FILE ?= $(CI_ARTIFACTS_DIR)/ci-integration-tests.status
BUILD_STATUS_FILE ?= $(CI_ARTIFACTS_DIR)/ci-build.status
UNIT_TEST_REPORT ?= $(CI_ARTIFACTS_DIR)/unit-tests.json
UNIT_COVERAGE_REPORT ?= $(CI_ARTIFACTS_DIR)/unit-coverage.txt
INTEGRATION_JSON_REPORT ?= $(CI_ARTIFACTS_DIR)/integration-report.json
INTEGRATION_JUNIT_REPORT ?= $(CI_ARTIFACTS_DIR)/integration-report.xml

.PHONY: ci-static-analysis
ci-static-analysis: manifests generate fmt vet lint ## Run static analysis steps used in CI.

.PHONY: ci-code-verify-report
ci-code-verify-report: ## Run static analysis, unit tests, integration tests and build emitting CI artifacts.
	mkdir -p $(CI_ARTIFACTS_DIR)
	rm -f $(STATIC_ANALYSIS_STATUS_FILE) $(UNIT_TEST_STATUS_FILE) $(INTEGRATION_TEST_STATUS_FILE) $(BUILD_STATUS_FILE) $(UNIT_TEST_REPORT) $(UNIT_COVERAGE_REPORT) $(CI_COVER_PROFILE) $(INTEGRATION_JSON_REPORT) $(INTEGRATION_JUNIT_REPORT)
	@set +e; \
	static_status=0; \
	unit_status=0; \
	...
	static_status=$$?; \
	printf '%s\n' "$$static_status" > $(STATIC_ANALYSIS_STATUS_FILE); \
	$(MAKE) test-unit-report COVER_PROFILE=$(CI_COVER_PROFILE) | tee $(UNIT_TEST_REPORT); \
	unit_status=$$?; \
	if [ -f $(CI_COVER_PROFILE) ]; then \
		go tool cover -func=$(CI_COVER_PROFILE) | tee $(UNIT_COVERAGE_REPORT); \
		coverage_status=$$?; \
		if [ "$$coverage_status" -ne 0 ] && [ "$$unit_status" -eq 0 ]; then \
			unit_status=$$coverage_status; \
		fi; \
	fi; \
	printf '%s\n' "$$unit_status" > $(UNIT_TEST_STATUS_FILE); \
	$(MAKE) test-integration-report JSON_REPORT=$(INTEGRATION_JSON_REPORT) JUNIT_REPORT=$(INTEGRATION_JUNIT_REPORT); \
	integration_status=$$?; \
	printf '%s\n' "$$integration_status" > $(INTEGRATION_TEST_STATUS_FILE); \
	$(MAKE) build-manager; \
	build_status=$$?; \
	printf '%s\n' "$$build_status" > $(BUILD_STATUS_FILE); \
	if [ "$$static_status" -ne 0 ] || [ "$$unit_status" -ne 0 ] || [ "$$integration_status" -ne 0 ] || [ "$$build_status" -ne 0 ]; then \
		exit 1; \
	fi

.PHONY: ci-e2e-report
ci-e2e-report: ## Build, push and run the E2E suite with machine-readable reports for CI.
	mkdir -p $(CI_ARTIFACTS_DIR)
...
test: manifests generate fmt vet ## Run unit tests.
	go test $$(go list ./... | grep -v /e2e | grep -v /test/integration) -coverprofile $(COVER_PROFILE) -count=1

.PHONY: test-unit-report
test-unit-report: manifests generate fmt vet ## Run unit tests emitting go test JSON for CI reporting.
	go test -json $$(go list ./... | grep -v /e2e | grep -v /test/integration) -coverprofile $(COVER_PROFILE) -count=1
...
GINKGO_FLAGS ?= -v --github-output

.PHONY: test-integration-report
test-integration-report: manifests generate fmt vet setup-envtest ## Run integration tests (envtest) emitting machine-readable reports for CI.
	KUBEBUILDER_ASSETS="$(shell $(ENVTEST) use $(ENVTEST_K8S_VERSION) --bin-dir $(LOCALBIN) -p path)" $(GINKGO) $(GINKGO_FLAGS) $(if $(JSON_REPORT),--json-report=$(JSON_REPORT)) $(if $(JUNIT_REPORT),--junit-report=$(JUNIT_REPORT)) ./test/integration
...
test-e2e-report: manifests generate fmt vet ## Run the e2e tests emitting machine-readable reports for CI.
	KIND_CLUSTER=$(KIND_CLUSTER_E2E) CERT_MANAGER_INSTALL_SKIP=true OPERATOR_IMAGE=${IMG} $(if $(GOMAXPROCS),GOMAXPROCS=$(GOMAXPROCS)) $(GINKGO) $(GINKGO_FLAGS) $(if $(JSON_REPORT),--json-report=$(JSON_REPORT)) $(if $(JUNIT_REPORT),--junit-report=$(JUNIT_REPORT)) ./test/e2e
...
.PHONY: build
build: manifests generate fmt vet test-all build-manager ## Build manager binary.
...
docker-build-image: ## Build docker image with the manager.
	$(CONTAINER_TOOL) build -t ${IMG} .

.PHONY: docker-build
docker-build: test-all docker-build-image ## Build docker image with the manager.
