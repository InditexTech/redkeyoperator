...
redis-benchmark: # Benckmark a redis cluster
	if [ "$(WORKSPACE)" == "default" ]; then \
  		kubectl exec -it -n $(NSTEST) redkey-cluster-0 -- redis-benchmark -c 100 -n 100000 -t set,get; \
	else \
	  if [ -z "$(REDIS_POD)" ]; then \
...
redis-insert:
	...
	if [ "$(WORKSPACE)" == "test" ]; then \
  		cat /tmp/data.txt | kubectl exec -i -n $(NSTEST) redkey-cluster-0 -- redis-cli -c; \
	else \
	  if [ -z "$(REDIS_POD)" ]; then \
