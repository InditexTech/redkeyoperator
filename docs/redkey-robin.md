...
Changes made to the RedkeyCluster object, managed directly by the Operator, such as changes in the number of primaries that involve scaling the cluster or changes in the configuration that require upgrading the cluster, are reported by the Operator to Robin through the API that the latter exposes.
