...
func (nodeList *ClusterNodeList) EnsureClusterSlotsStable(ctx context.Context, log logr.Logger) error {
	...
	for _, node := range masterNodes {
		...
		if err := slotsInMigratingButNoNodeImporting(ctx, node, nodeList, log); err != nil {
...
func slotsInMigratingButNoNodeImporting(ctx context.Context, node *ClusterNode, nodeList *ClusterNodeList, log logr.Logger) error {
...
func (nodeList *ClusterNodeList) AssignMissingSlots() error {
	...
	slotsPerNode := int(math.Ceil(float64(16384 / float64(len(masterNodes)))))
...
func CalculateSlotMoveMap(slotMap map[*ClusterNode][]int, options *MoveMapOptions) (map[*ClusterNode][]int, error) {
	...
	slotsPerWeight := totalSlots / totalWeight
...
func assignSlotsToNode(node *ClusterNode, slots []int, slotsPerWeight int, options *MoveMapOptions, result map[*ClusterNode][]int) {
	...
	nodeShouldSend := len(slots) > shouldHaveSlots+(shouldHaveSlots*options.threshold/100)
...
func CalculateMoveSequence(SlotMap map[*ClusterNode][]int, SlotMoveMap map[*ClusterNode][]int, options *MoveMapOptions) []MoveSequence {
	...
	slotsPerWeight := int(float64(totalSlots / totalWeight))
...
// Calculates the rate of unbalance by getting the maximum and minimum number of slots
// in a node as a percentage.
func (nodeList *ClusterNodeList) GetUnbalanceRate(ctx context.Context) (float64, error) {
	// Calculate max and min slots in a node
	slotMap, err := nodeList.buildSlotMap()
	if err != nil {
		return 0, err
	}
	...
	// Protect against degenerated situations
	if max == 0 {
		return 0, fmt.Errorf("dividing by zero in GetUnbalancedRate")
	}

	// Balance rate calculated from max and min obtained
	rate := ((float64(max) - float64(min)) * 100) / float64(max)
